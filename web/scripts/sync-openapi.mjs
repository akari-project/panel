...
async function fetchFile(repo, path) {
  ...
  const url = `https://raw.githubusercontent.com/${lock.repository}/${ref}/${path}`;
...
let commit = null;
if (repo) {
  commit = execFileSync('git', ['-C', repo, 'rev-parse', `${ref}^{commit}`], { encoding: 'utf8' }).trim();
}
...
console.log(`openapi: ${ref}${commit ? ` (${commit.slice(0, 12)})` : ''} 来自 ${repo ?? 'GitHub'}`);
